...
2. **Set Workers Secrets** for the production Worker: `npx wrangler secret put SUPABASE_URL` and `npx wrangler secret put SUPABASE_KEY`. Use the project Supabase service-role values, not anon. Verify via `npx wrangler secret list`.
...
7. **First deploy**: `npx wrangler deploy` from `master` (or via the GH Actions workflow auto-deploying on merge per `tech-stack.md`).
