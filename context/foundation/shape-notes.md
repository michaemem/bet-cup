...
created: 2026-05-19
updated: 2026-05-19  # 20:53 - targeted Socratic pass on FR-007/013/015/017; FR-022 added
...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "primary persona"
      decision: "the admin/organizer running the pool"
    - topic: "insight"
      decision: "private friend-pool is a different shape from public leaderboard-driven apps (Kicktipp, Superbru)"
    - topic: "persona scope"
      decision: "a specific known group — one tournament, one admin, single tenant"
    - topic: "auth shape"
      decision: "admin creates accounts; participants log in with credentials + password; single admin role"
    - topic: "role separation"
      decision: "two roles — admin and participant"
    - topic: "credential handoff"
      decision: "admin sets initial password, shares out-of-band; user can change after first login"
    - topic: "prediction lock"
      decision: "hard cutoff at scheduled kickoff; after kickoff predictions become visible and uneditable"
    - topic: "scoring rule"
      decision: "Kicktipp-style: 3 exact / 2 goal-diff / 1 outcome / 0 otherwise (hardcoded in MVP)"
    - topic: "timeline"
      decision: "3 weeks after-hours; scope deemed appropriate"
    - topic: "FR-007 bulk match entry (Socratic)"
      decision: "kept one-by-one; added FR-022 for bulk-paste entry alongside it"
    - topic: "FR-013 prediction editability (Socratic)"
      decision: "kept editable until kickoff; single-shot rejected as punitive for friend-group use"
    - topic: "FR-015 admin-blind to predictions (Socratic)"
      decision: "kept UI-blindness; encrypt-at-rest rejected as too costly; trust model assumes admin won't read DB"
    - topic: "FR-017 admin can play (Socratic)"
      decision: "kept admin-plays; FR-014 + FR-015 protect fairness; separate-account workaround rejected as out-of-touch with friend-group usage"
  frs_drafted: 22
...
> Source seed: `idea-notes.md` (BetCup MVP brief). This document is the structured input to `/10x-prd`.
...
- FR-007: Admin can add a match to the tournament by entering home team, away team, and kickoff date and time. Priority: must-have
  > Socrates: Counter-argument considered: "tournaments have 48–64 matches; entering one-by-one is painful and error-prone." Resolution: kept as written but FR-022 (bulk paste entry) added alongside it, so the admin has both flows.
...
- **External sports/fixtures API integration.** The admin enters every match and every result by hand. No fetch from football-data.org, ESPN, OpenLigaDB, or similar. — Rationale: every external integration is a 1-week tax on a 3-week project; manual entry is acceptable for one tournament.
...
- **Native mobile app.** The web app must be usable on mobile browsers, but no iOS/Android native app, no PWA install prompt, no mobile-specific UI work. — Rationale: scope discipline.
- **Real-time updates.** No live leaderboard streaming, no live match-score feed, no WebSocket layer. The leaderboard updates the next time a participant loads the page after the admin enters a result. — Rationale: real-time has high implementation cost relative to the rare moments it would be visible.

## Forward: tech-stack

Informational notes captured during shape; NOT part of PRD. Pass these to the tech-stack-selection step downstream.

- **Avoid: PHP** — explicit user preference.
- **Avoid: Java / Spring** — explicit user preference.
- **Stack remains otherwise open** — no other framework, language, database, or hosting commitments captured here. The product priors (single-tenant web app, ~5–20 users, 3-week MVP, no external APIs, no real-time, no notifications) imply a modest stack but the choice is for the next step.

## Quality cross-check

Run on 2026-05-19. All six greenfield cross-check elements present. No gaps to surface to `/10x-prd`'s `## Open Questions`.

- Access Control — present
- Business Logic — present
- Project artifacts — present
- Timeline-cost ack — present (within 3-week budget; no acknowledgment block needed)
- Non-Goals — present
- Preserved behavior — n/a (greenfield)

Status: `accepted`.

Note: the per-FR Socratic challenge round (Step 4.5) was deferred by user choice — Socratic challenge is not part of the cross-check, but worth a re-pass before `/10x-frame` if any FR is contested during planning.
