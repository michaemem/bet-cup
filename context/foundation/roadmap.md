...
| S-09 | admin-reset-participant-password | admin resets any participant's password (system-generated temp); the participant's sessions are revoked and they sign in with the temp password, then change it in `/settings` | S-01, F-01 | FR-024 (relates FR-002, FR-003) | done |
...
- **Outcome:** admin views a kickoff-passed match with no result entered, enters home/away scores and confirms; every participant's prediction for that match is scored per FR-018 (5 / 3 / 2 / 0); the post-kickoff predictions become visible to all participants (FR-016); the leaderboard ranks all participants by total points across all played matches and reflects the new totals immediately. If the admin re-enters the result, all affected per-prediction scores recompute and the leaderboard updates.
...
- **Unknowns:**
  ...
  - Leaderboard tie-break rule: **RESOLVED 2026-05-28 (#9).** Rank by total points; break ties by exact-score-prediction count (more 5-pointers wins); if still tied, alphabetical by participant name (case-insensitive, ascending). See `## Open Roadmap Questions` §1.
...
1. **Leaderboard tie-break rule.** ✅ **RESOLVED 2026-05-28 ([#9](https://github.com/michaemem/bet-cup/issues/9)).** Rank by total points; break ties by exact-score-prediction count (more 5-pointers wins); if a tie remains, fall back to alphabetical by participant name (case-insensitive, ascending). Reflected in `S-04` and PRD FR-020.
...
- **S-04: admin views a kickoff-passed match with no result entered, enters home/away scores and confirms; every participant's prediction for that match is scored per FR-018 (5 / 3 / 2 / 0); the post-kickoff predictions become visible to all participants (FR-016); the leaderboard ranks all participants by total points across all played matches and reflects the new totals immediately. If the admin re-enters the result, all affected per-prediction scores recompute and the leaderboard updates.** — Archived 2026-06-05 → `context/archive/2026-06-04-results-scoring-leaderboard/`. Lesson: —.
...
- **S-09: from the existing "manage participants" view, the admin triggers a password reset for any participant; the system generates a new random temporary password that the admin copies and shares with the participant out-of-band (the same handoff model as the S-01 initial password). The reset revokes the participant's existing sessions, the old password stops working, and the participant signs in with the temporary password and can then change it themselves on the existing `/settings` page (no forced change-on-next-login).** — Archived 2026-06-09 → `context/archive/2026-06-08-admin-reset-participant-password/`. Lesson: —.
